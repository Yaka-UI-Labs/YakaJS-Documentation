...
export function NavListHeading({ children, level = 3 }: React.PropsWithChildren<{ level?: HeadingLevel }>) {
  ...
  return (
    <Element className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm py-2 font-mono text-sm/6 font-semibold tracking-widest text-gray-700 uppercase sm:text-xs/6 dark:bg-gray-950/95 dark:text-gray-300">
      {children}
    </Element>
  );
}
...
export function NavListItems({ children, nested = false }: React.PropsWithChildren<{ nested?: boolean }>) {
  return (
    <ul
      className={clsx(
        "flex flex-col gap-1.5 border-l transition-colors",
        nested 
          ? "border-transparent ml-3" 
...
export function NavListItem({ children }: React.PropsWithChildren) {
  return <li className="-ml-px flex flex-col items-start gap-1.5">{children}</li>;
}
...
export function NavListLink({
  ...
  return (
    <CloseButton
      as={Link}
      className={clsx(
        "group relative inline-flex items-center gap-2 border-l-2 text-base/7 transition-all duration-200 ease-out sm:text-sm/6",
        // Default state
        ...
        // Padding
        nested ? "pl-7 sm:pl-6" : "pl-4 sm:pl-3.5",
      )}
      href={href}
      {...props}
    >
      ...
      <span className={clsx(
        "absolute left-0 w-1.5 h-1.5 rounded-full transition-all duration-200",
      ...
      <span className={clsx(
        "absolute inset-y-0 left-0 w-0 bg-gradient-to-r from-blue-50 to-transparent transition-all duration-200",
